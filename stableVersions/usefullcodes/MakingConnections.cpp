...
struct Game {
    ...
    int getReservedUnits(int armyIndex) // просмотр всех зарезервированных на ход юнитов
    {
        int sum = 0;
        for (auto &cmd : playerCommands)
        {
            if (cmd.armyIndex == armyIndex)
                sum += cmd.units;
        }
        return sum;
    }
    int getAvailableUnits(int armyIndex) // вычисление доступных для нового действия
    {
        return armies[armyIndex].soldiers - getReservedUnits(armyIndex);
    }
    ...
    void processEvents() {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed)
            {
                ...
                if (event.mouseButton.button == sf::Mouse::Left && hoveredCity != NULL)
                {
                    ...
                    if (connectionMode)
                    {
                        ...
                        // первый клик
                        if (firstCityForConnection == -1)
                        {
                            firstCityForConnection = city.id;
                            std::cout << "First city: " << city.name << "\n";
                        }
                        else
                        {
                            int a = firstCityForConnection;
                            int b = city.id;

                            // защита от дублей
                            if (std::find(cities[a].neighbors.begin(),cities[a].neighbors.end(), b) == cities[a].neighbors.end())
                            {
                                cities[a].neighbors.push_back(b);
                                cities[b].neighbors.push_back(a);
                            }

                            std::cout << "Connected: " << cities[a].name<< " <-> " << cities[b].name << "\n";

                            firstCityForConnection = -1;
                        }
            ...
            if (showPopup && event.type == sf::Event::KeyPressed)
            {
                ...
                {
                    int armyIndex = playerCommands[selectedCommandIndex].armyIndex;
                    int maxUnits = getAvailableUnits(armyIndex) + playerCommands[selectedCommandIndex].units;
            ...
            if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::C)
            {
                ...
                firstCityForConnection = -1;
                std::cout << "Connection mode: " << connectionMode << "\n";
            }
            if (connectionMode && event.key.code == sf::Keyboard::Enter)
            {
                std::ofstream file;
    ...
    void render() {
        ...
        for (auto &c : cities)
        {
            ...
            // !!!!!! связи
                for (int nid : c.neighbors)
                {
                    // найти город по id
                    auto it = std::find_if(cities.begin(), cities.end(),
                        [&](const City& x){ return x.id == nid; });

                    if (it == cities.end()) continue;

                    sf::Vertex line[] =
                    {
                        sf::Vertex(c.position, sf::Color::Red),
                        sf::Vertex(it->position, sf::Color::Red)
                    };
